...
(function() {
    ...
    class TicketHandler {
        ...
        handleVerification() {
            ...
            for (const [bankName, code] of Object.entries(CONFIG.BANK_CODES)) {
                if (textContent.includes(bankName)) {
                    if ((bankName === "國泰世華" && textContent.includes("卡號前8碼")) ||
        ...
        _processGameOrders(gameList) {
            ...
            // 嘗試按照指定順序點擊
            for (const index of CONFIG.BUY_DATE_INDEXES.map(i => i - 1)) {
            ...
            for (const seatPattern of CONFIG.BUY_AREA_SEATS) {
                ...
                for (const element of randomizedElements) {
                    ...
                    const isMatch = keywords.length === 0 || 
                                  keywords.every(keyword => element.textContent.includes(keyword));
        ...
        _selectTicketQuantity() {
            ...
            if (ticketPriceSelect) {
                ...
                ticketPriceSelect.value = hasDesiredCount ? CONFIG.BUY_COUNT : 
